...
configure_zsh_neovim() {
  local SOURCE_LINE='export EDITOR=nvim'
  if ! grep -qF "$SOURCE_LINE" ~/.zshrc 2>/dev/null; then
    info "Adding nvim as default editor to ~/.zshrc..."
    echo "" >>~/.zshrc
    echo "# default editor config (used by git and bundle and others)" >>~/.zshrc
    echo "$SOURCE_LINE" >>~/.zshrc
  else
    info "default editor is already set to nvim"
  fi
}
